...
# dev-v0.1.2 - MEDIA_FILES_PATH now sourced from .env; /mnt/ebs is no longer hardcoded

#####
# v0.1.2 - OPTIONAL STORAGE PATH
#   - MEDIACMS_ROOT defined before EBS_PATH so .env can be sourced first
#   - EBS_PATH now reads MEDIA_FILES_PATH from .env (falls back to /mnt/ebs/mediacms_media)
#   - Replaced hardcoded 'df -h /mnt/ebs' with 'df -h "$EBS_PATH"' (filesystem-aware)
#   - /mnt/ebs is no longer required; any host path set in .env is fully supported
#   - DOMAIN sourced dynamically from .env for the curl check (no hardcoded domain)
#   - Fixed: replaced tr quote-strip with sed to avoid multi-layer quoting bugs
#   - All validation and initialization logic unchanged
...
# Usage: ./scripts/init_validate_storage.sh [--init|--validate|--full]
#   --init      : Initialize storage structure only
...
# ============================================
# CONFIGURATION
# MEDIACMS_ROOT must be defined first so .env can be read below
# Per foundational requirements, project root is always /mediacms
# ============================================
MEDIACMS_ROOT="/mediacms"
OLD_MEDIA_FILES="$MEDIACMS_ROOT/media_files.old.20260125"

# ============================================
# STORAGE PATH - sourced from .env
# Reads MEDIA_FILES_PATH from .env if present
# Falls back to /mnt/ebs/mediacms_media as the default
# To use a different path, update MEDIA_FILES_PATH in .env
# sed strips leading/trailing whitespace and surrounding double-quotes only
# ============================================
_MEDIA_FILES_PATH=""
if [ -f "$MEDIACMS_ROOT/.env" ]; then
    _MEDIA_FILES_PATH=$(grep -E '^MEDIA_FILES_PATH=' "$MEDIACMS_ROOT/.env" \
        | head -1 \
        | cut -d'=' -f2- \
        | sed 's/^[[:space:]]*//; s/[[:space:]]*$//; s/^"//; s/"$//')
fi

if [ -n "$_MEDIA_FILES_PATH" ]; then
    EBS_PATH="$_MEDIA_FILES_PATH"
    echo -e "${GREEN}[config]${NC} Using MEDIA_FILES_PATH from .env: $EBS_PATH"
else
    EBS_PATH="/mnt/ebs/mediacms_media"
    echo -e "${YELLOW}[config]${NC} MEDIA_FILES_PATH not found in .env -- using default: $EBS_PATH"
fi
...
initialize_storage() {
    echo -e "${BLUE}===========================================================\${NC}"
    echo -e "${BLUE}MediaCMS Storage Initialization${NC}"
    echo -e "${BLUE}Storage Path: $EBS_PATH${NC}"
    echo -e "${BLUE}===========================================================${NC}"
    echo ""
    ...
    echo -e "${YELLOW}[2/7]${NC} Creating directory structure at $EBS_PATH..."
    ...
    echo "   Storage directory contents ($EBS_PATH):"
...
validate_storage() {
    ...
    echo -n "Storage path exists ($EBS_PATH): "
    if [ -d "$EBS_PATH" ]; then
        echo -e "${GREEN}✅${NC}"
    else
        echo -e "${RED}❌${NC}"
    ...
    echo -n "Storage path has content: "
    ...
    if [ "$SERVICES_RUNNING" = true ]; then
        ...
        mountpoint -q /home/mediacms.io/mediacms/media_files && echo '✅' || echo '❌'

        echo -n 'media_files has content: '
        [ \"\$(ls -A /home/mediacms.io/mediacms/media_files)\" ] && echo '✅' || echo '❌'

        echo -n 'HLS directory exists: '
        [ -d /home/mediacms.io/mediacms/media_files/hls ] && echo '✅' || echo '⚠️'

        echo -n 'Original files directory exists: '
        [ -d /home/mediacms.io/mediacms/media_files/original ] && echo '✅' || echo '❌'

        echo -n 'Thumbnails directory exists: '
        [ -d /home/mediacms.io/mediacms/media_files/thumbnails ] && echo '✅' || echo '❌'

        echo -n 'CyTube manifests directory exists: '
        [ -d /home/mediacms.io/mediacms/media_files/cytube_manifests ] && echo '✅' || echo '⚠️'
        "
    else
    ...
    if [ "$SERVICES_RUNNING" = true ]; then
        # Load DOMAIN dynamically from .env - sed strips whitespace and double-quotes only
        _DOMAIN=""
        if [ -f "$MEDIACMS_ROOT/.env" ]; then
            _DOMAIN=$(grep -E '^DOMAIN=' "$MEDIACMS_ROOT/.env" \
                | head -1 \
                | cut -d'=' -f2- \
                | sed 's/^[[:space:]]*//; s/[[:space:]]*$//; s/^"//; s/"$//')
        fi
        if [ -n "$_DOMAIN" ]; then
            ...
            HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "https://$_DOMAIN/static/js/cytube-export.js" 2>/dev/null)
            ...
            echo -e "${YELLOW}⚠️  DOMAIN not set in .env -- skipping cytube-export.js check${NC}"
        fi
    fi
    echo ""
...
print('HLS_ENABLE: ' + str(getattr(settings, 'HLS_ENABLE', 'NOT SET')))
" 2>/dev/null
        echo ""
    fi

    # Section 5: Storage Status
    # Uses $EBS_PATH directly - no hardcoded mount point
    echo -e "${YELLOW}5. Storage Status${NC}"
    echo "-----------------"
    echo "Storage filesystem usage ($EBS_PATH):"
    df -h "$EBS_PATH" 2>/dev/null | tail -1
    echo ""
...
main() {
    ...
    if [ "$MODE" == "init" ]; then
        ...
        echo "  - Monitor encoding: docker logs -f mediacms-celery_worker-1"
        echo "  - Check storage:    df -h $EBS_PATH"
        echo "  - Run validation:   $0 --validate"
    fi
    echo ""
}
