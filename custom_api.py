...
# - Added CYTUBE_DESCRIPTION and CYTUBE_ORGANIZATION support
# - All HTTPS URLs now constructed from DOMAIN env var
# - Removed manual Find/Replace requirement for domain
...
# v0.3.1 - Better search logic for domain counts
...
# Configuration now centralized in .env file (DOMAIN, CYTUBE_DESCRIPTION, CYTUBE_ORGANIZATION)
...
if MISSING_VARS:
    raise EnvironmentError(
        f"❌ CRITICAL: Missing required environment variables in .env: {', '.join(MISSING_VARS)}\n"
        f"   Please configure these in your .env file before starting MediaCMS.\n"
        f"   See .env file for MANUAL CONFIGURATION SECTION."
    )

# Load environment variables with validation
DOMAIN = os.environ.get('DOMAIN')
CYTUBE_DESCRIPTION = os.environ.get('CYTUBE_DESCRIPTION', 'Custom MediaCMS streaming server')
CYTUBE_ORGANIZATION = os.environ.get('CYTUBE_ORGANIZATION', 'MediaCMS-CyTube')
...
if DOMAIN:
    ...
    print(f"✅ Custom API initialized with DOMAIN: {DOMAIN}")
    print(f"✅ CyTube Organization: {CYTUBE_ORGANIZATION}")
else:
    print(f"⚠️ WARNING: DOMAIN not set in environment")
...
def generate_cytube_manifest(request, friendly_token):
    ...
    try:
        ...
        if master_playlist_path:
            ...
            # Build the HTTPS URL for the master playlist using DOMAIN from environment
            hls_url = f"https://{DOMAIN}/media/hls/{hls_dir_name}/master.m3u8"
        ...
        manifest = {
            ...
            "meta": {
                "description": media.description or CYTUBE_DESCRIPTION,
